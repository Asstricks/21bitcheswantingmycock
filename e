...
elseif type == "Slider" then
    ...
    -- **Replaced SliderButton.MouseButton1Down:Connect Function Starts Here**
    ...
    -- **Replaced SliderButton.MouseButton1Down:Connect Function Ends Here**
...
-- Debugging: Check where the script is running from and inspect its parent
print("Script is running from: ", script.Parent)

-- Try to get references to MainFrame and TopBar, and print debug info
local mainFrame = script.Parent:FindFirstChild("MainFrame")
if mainFrame then
    print("MainFrame found!")
else
    warn("MainFrame not found! Please make sure MainFrame exists in the parent.")
end

local topBar = mainFrame and mainFrame:FindFirstChild("TopBar") or nil
if topBar then
    print("TopBar found!")
else
    warn("TopBar not found! Please make sure TopBar exists inside MainFrame.")
end

-- If MainFrame or TopBar don't exist, stop executing further
if not mainFrame or not topBar then
    return -- Stop further execution if references are missing
end

-- Create buttons for Minimize, Maximize, and Exit
...
-- Button creation
local minimizeButton = createButton("MinimizeButton", "-", UDim2.new(1, -60, 0, 5))
local maximizeButton = createButton("MaximizeButton", "+", UDim2.new(1, -60, 0, 5))
maximizeButton.Visible = false -- Initially hidden
local exitButton = createButton("ExitButton", "X", UDim2.new(1, -30, 0, 5))
...
local function minimizeUI()
    if mainFrame.Size == originalSize then
        TweenService:Create(mainFrame, TweenInfo.new(0.3), {Size = UDim2.new(0, 700, 0, topBar.Size.Y.Offset)}):Play()
...
exitButton.MouseButton1Click:Connect(exitUI)

-- Make TopBar draggable (if not already)
